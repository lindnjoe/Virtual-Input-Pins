...
# Poll runout and speed monitors once per second
...
class OAMSManager:
    ...
    def start_monitors(self):
        ...
        for (fps_name, fps_state) in self.current_state.fps_state.items():
            ...
            # Runout monitoring needs to fire promptly, so register it first
            ...
            # Delay other monitors slightly to avoid delaying runout checks
            when = reactor.NOW + MONITOR_ENCODER_PERIOD
            self.monitor_timers.append(
                reactor.register_timer(self._monitor_unload_speed_for_fps(fps_name), when)
            )
            self.monitor_timers.append(
                reactor.register_timer(self._monitor_load_speed_for_fps(fps_name), when)
            )
